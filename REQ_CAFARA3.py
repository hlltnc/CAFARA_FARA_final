...
from DROP_overlap import drop_late_requests_by_overlap
...
#F_threshold = 0.7  # örnek bir eşik değeri
        ...
        #df_sorted, dropped_overlap, df_marked = drop_late_requests_by_overlap(
        # df_sorted,
        # overlap_threshold=1
        #)

        #drop_count_overlap = dropped_overlap
...
# Skaler kapasiteyi 'capacity' alanına yaz (listeyi ezmeyelim)
    ...
    # 3. Path ve hesaplamalar
            ...
            for path in paths:
                ...
                required_thresh_1 = calculate_required_threshold(f_threshold, path)
                
                F_initial_2 = 0.5 + (F_initial - 0.5) * np.exp(-alpha * L)

                rounds_0 , F_final_0 = calculate_purification_rounds(F_initial_2, required_thresh_1)

                res = select_required_F_and_M(
                path=path,
                lookup_df=lookup_df_real,
                C_rel_design=Crel_design,
                L_per_hop_km=fixed_distance,        # hop başına km
                selection_policy="max"      # ulaşılamazsa max FE2E fallback
                )

                F_target = max(required_thresh_1, res["F_target"])
                ...
                M_used   = res["M_selected"]
                print(f"[DBG] X={res['X']} L={res['L_matched_km']}km  "
                f"M_used={M_used}  F_target={F_target:.4f}  "
                f"(reason={res['reason']}, C_rel_design={res['C_rel_design']:.4f})")
                required_thresh=  F_target 
                ...
                rounds_needed_dizin.append(rounds)
...
# --- Uçtan-uca (path) entanglement generation rate ---
                ...
                "F_final": F_final_path,   # bundan sonra F_final = path final fidelity
                "t_ent_gen": t_ent_gen,
                "t_purif": t_purif,
                "EGR": route_res,
                "EGR_R_route": EGR_R,
...
# ---- C_real’i her path için ekle ----
    ...
    # ----- Buradan sonrası sadece decision="ok" için -----
        ...
        drop_rate_overlap = 100.0 * drop_count_overlap / total_requests
...
# kategori bazlı listeler (bu listeleri sabit başta tanımlıyorsun; bkz. aşağıdaki not)
        ...
        #drop_rates_overlap.append(drop_rate_overlap)
...
# --- tekrarlardan (repetition) ortalama al ve üst düzey dizilere koy ---
    ...
    avg_drop_overlap = _mean_or_nan(drop_rates_overlap)
    ...
    all_avg_drop_overlap.append(avg_drop_overlap)
...
def save_results_npz(
    ...
    All_req_t_ent,
    All_req_t_purif,
    out_path="REQ_YC_Cth_07_C100_npz/REQ1_metrics.npz"
):
    """
